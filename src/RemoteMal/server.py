...
class RemoteServer(object):
    ...
    def PartitionTable(self):
        ...
        for data in PathLib.remover(self.partitions_info.split("<part>")):
            table_data.append(data.split("<info>"))
    ...
    def WinBackDir(self,cmd):
        ...
        self.current_path=self.path
        self.send(self.path)
        drive,cwd=DriveAPath(self.path,self.partitions)        #return the path and drive in the absolute path
        self.path=BackDir(drive,cwd,cmd[3:len(cmd)])
...
if __name__=="__main__":
    ...
    r.PartitionsInfo()
    r.DisplayPartitionInfo(r.PartitionTable())
