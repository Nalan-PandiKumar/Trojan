...
class RemoteServer(object):
    ...
    def Getstdin(self):
        ...
        while(True):
            ...
            if(flag.upper()=="Y"): #check the flag is Y for Yes
                self.send(flag)
                stdin=input("\x1b[33mInput:\x1b[0m")
                self.send(stdin)
                break
                ...
                print("Wrong input:(Y or N)") #Any other input considered as wrong
    ...
    def main(self):
        ...
        if(self.platform=="win32"):
            ...
            while(True):
                #Tracking Added and removed partitions

                self.PartitionsInfo()
                new_data=self.PartitionTable()
                self.TrackPartition(self.table_data,new_data)

                cmd=input(f"{self.path}>")
                self.send(cmd)

                try:
                    if((cmd.split()[0]=="cd") and (len(cmd.split())>=2 )and("../" in cmd)):
                    ...
                    #set the path variable to its previous state
                    self.path=self.current_path
            ...
            while(True):
                #Tracking newly added removed partition
